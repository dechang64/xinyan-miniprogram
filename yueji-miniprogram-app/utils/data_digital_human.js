// 悦济 v3.1.x — 先哲数字人 5 张国画头像 (老子/孔子/周文王/岐伯/元神, 后续云存储)
// 严守: 纯国画 + 文字, 0 出现禁用词
...
const DIGITAL_HUMAN_AVATARS = {
  laozi: {
    ...
    question: '你最近心里有什么牵绊?',
    // v3.1 阶段 5: 角色差异化 (调性 + 标志性 phrase)
    tag: '玄思 · 慢',
    persona: {
      speed: 'slow',         // 慢, 留白
      maxLength: 80,
      signature: ['上善若水', '道可道', '致虚极', '归根曰静'],
      style: '玄思, 像水, 不评判, 多引用, 留白多',
    },
    responses: {
  ...
  zhouwenwang: {
    ...
    persona: {
      speed: 'medium',
      maxLength: 100,
      signature: ['需', '潜龙勿用', '天行健', '同人于野'],
      style: '沉稳, 像山, 讲规律, 引用卦辞爻辞, 不预测',
    },
    responses: {
  ...
  qibo: {
    ...
    persona: {
      speed: 'medium',
      maxLength: 100,
      signature: ['起居有常', '形劳而不倦', '五谷为养', '法于阴阳'],
      style: '温和, 像春天, 重起居/情志/四时, 不开药',
    },
    responses: {
  ...
  yuanshen: {
    ...
    tag: '静默 · 留白',
    persona: {
      speed: 'very-slow',     // 极慢, 一句一顿
      maxLength: 60,
      signature: ['常清静', '心无其心', '形无其形', '物无其物'],
      style: '静默, 留白, 一句一顿, 不解释不教, 极简',
    },
    responses: {
