"""悦济 v0.7 — 12 星座 + 太阳/月亮/上升 3 星座 + 元素 + 4 主题能量
...
def calc_moon_sign(birth_year: int, birth_month: int, birth_day: int, hour: int) -> str:
    ...
    真实月亮星座需要精确到秒 + 出生地经纬度, 悦济用「生日 + 出生时辰」近似估算。
...
def calc_rising_sign(hour: int) -> str:
    ...
    真实上升星座需要精确到秒 + 出生地经纬度, 悦济用「出生时辰」近似估算。
    严守声明: 这是「参考值」, 不是精确天文计算。
    """
    # 时辰对应
...
星座是西方占星文化的一部分, 悦济只做「性格倾向 / 元素平衡」参考, 不做「运势预测」。
...
数据来源: 12 星座表 (悦济自维护) + sxtwl 农历换算 (月亮星座近似估算)
