"""悦济 v0.7.1.4 — 八字四柱 (纯 Python 实现, 不依赖 sxtwl)
...
# 24 节气表 (2026 年简化版, 用月份近似 — 实际悦济 PRD 只算 4 柱, 不算节气精确)
...
def _calc_month_gan_zhi(year: int, month: int, day: int) -> tuple:
    ...
    实际精确算法要按节气切月 (立春换年, 惊蛰换寅月, ...) — 悦济只算 4 柱不精算
    """
...
def calc_bazi(year: int, month: int, day: int, hour: int) -> dict:
    ...
    悦济 PRD §3 明确: 八字只算 4 柱 + 元素, 不算大运流年.
    """
...
八字是中国传统命理文化的一部分, 悦济只做「文化参考」, 不做「命理预测」。

悦济只用做「性格倾向 + 元素平衡」自评参考, 严守:
- 不宣称决定命运
- 不替代心理咨询
- 不引导用户「必须」/「应该」
- 输出只说「倾向」/「可能」/「参考」
...
- 悦济 PRD §3: 八字只算 4 柱 + 元素, 不算大运流年 → 简化算法完全够用
"""